...
class TPO:
    ...
    def __init__(
        self,
        pretrained: str = "ruili0/LLaVA-Video-7B-Qwen2-TPO",
    ...
    def stream_generate_until(self, requests: dict, gen_kwargs: dict) -> List[str]:
        ...
        if (
            ...
            if task_type == "image":
                ...
                image_tokens = (
                    [DEFAULT_IMAGE_TOKEN] * len(spare_frames)
                    if self.token_strategy == "multiple"
                    else [DEFAULT_IMAGE_TOKEN]
                )
                pre_prompt= f"The video lasts for {video_time:.2f} seconds, and {len(video[0])} frames are uniformly sampled from it. These frames are located at {frame_time}.Please answer the following questions related to this video."
...
if __name__ == "__main__":
    ...
    model = TPO(pretrained="ruili0/LLaVA-Video-7B-Qwen2-TPO", model_name="llava_qwen", device_map=args.device)
    if args.image_path:
